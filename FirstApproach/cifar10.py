...
# Constants for my project (c) Vladimir
GRAD_FLOW = True
...
def inference(images, penalty=None):
  ...
  with tf.variable_scope('conv1') as scope:
    coef = _variable_on_cpu('coef', [], tf.constant_initializer(0.04))
    kernel = coef * reduction(_variable_with_weight_decay('weights',
                                         shape=[5, 5, 3, 64],
                                         stddev=3, wd2=0.001))
  ...
  with tf.variable_scope('local3') as scope:
    ...
    coef = _variable_on_cpu('coef', [], tf.constant_initializer(0.04))
    weights = coef * reduction(_variable_with_weight_decay('weights', shape=[dim, 384],
                                          stddev=3.04, wd2=0.001))
  ...
  with tf.variable_scope('local4') as scope:
    ...
    coef = _variable_on_cpu('coef', [], tf.constant_initializer(0.04))
    weights = coef *  reduction(_variable_with_weight_decay('weights', shape=[384, 192],
                                          stddev=3.04, wd2=0.001))
  ...
  with tf.variable_scope('softmax_linear') as scope:
    ...
    coef = _variable_on_cpu('coef', [], tf.constant_initializer(0.04))
    weights = coef * reduction(_variable_with_weight_decay('weights', [192, NUM_CLASSES],
                                          stddev=3.04, wd2=0.001))
...
def train(total_loss, global_step):
  ...
  with tf.control_dependencies([loss_averages_op]):
    ...
    grads = opt.compute_gradients(total_loss) # + tf.add_n(tf.get_collection('l1losses')) * CUSTOM_LOSS * tf.maximum(global_step - CUSTOM_START_STEP, 0) )
